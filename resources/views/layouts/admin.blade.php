...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-10 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        <ul class="space-y-2">
            <li class="border-b" data-app="changeDepartment">
                <form method="POST" action="{{ url('/admin/settings/active-department') }}">
                    @csrf
                    @method('PATCH')
                    <select v-cloak @change="$el.submit()" :value='@json(settings()->getActiveDepartment())' name="department" class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
